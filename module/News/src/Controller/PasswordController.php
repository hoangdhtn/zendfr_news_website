...
use News\Form\Auth\ForgotForm;
use News\Form\Auth\ResetForm;
...
class PasswordController extends AbstractActionController
{
	...
	public function forgotAction()
	{
		...
		return (new ViewModel(['form' => $forgotForm]))->setTemplate('news/auth/forgot');
	}
	...
	public function resetAction()
	{
		...
		$verify = $this->forgotTable->fetchToken($token, (int)$info->getId());
		if(!$verify) {
			...
			if($resetForm->isValid()) {
				try {
					...
					if($this->usersTable->updatePassword($data['new_password'], (int) $info->getId())) {
						$this->forgotTable->deleteToken((int)$info->getId());
					}
		...
		return (new ViewModel(['form' => $resetForm]))->setTemplate('news/auth/reset');
