...
use News\Form\Setting\EmailForm;
use News\Form\Setting\PasswordForm;
use News\Form\Setting\UploadForm;  # <- be sure to add this
use News\Form\Setting\UsernameForm;
		...
		if($request->isPost()) {
			...
			if($deleteForm->isValid()) {
				if($request->getPost()->get('delete_account') == 'Yes') {
					$this->usersTable->deleteAccount((int) $this->authPlugin()->getId());
					$this->flashMessenger()->addSuccessMessage('Tài khoản đã xóa.');
		...
		if($request->isPost()) {
			...
			if($passwordForm->isValid()) {
				...
				if($hash->verify($data['current_password'], $this->authPlugin()->getPassword())) {
					...
					/**
					  return $this->redirect()->toRoute('profile', ['id' => $this->authPlugin()->getId(), 'username' => mb_strtolower($this->authPlugin()->getUsername())]);
					*/
				}
